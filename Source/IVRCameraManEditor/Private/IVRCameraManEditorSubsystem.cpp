...
bool             UIVRCameraManEditorSubsystem::IVR_EnableTurboRecording;
float            UIVRCameraManEditorSubsystem::IVR_TurboRate           ;
...
void UIVRCameraManEditorSubsystem::ReadCameraManDefaultValues(FString& pIVR_RecordingPath,
	...
	float& pIVR_Gamma,
	bool& pIVR_EnableTurboRecording,
	float& pIVR_TurboRate,
	...
	float& pIVR_BaseLookUpRate)
{
	...
	if (FPlatformFileManager::Get().GetPlatformFile().FileExists(*SessionConfigFilePath))
	{
		...
		{
			pIVR_Width = atof(TCHAR_TO_UTF8(*tStr));
		...
		{
			pIVR_Height = atof(TCHAR_TO_UTF8(*tStr));
			UE_LOG(LogTemp, Log, TEXT("%s"), *(tStr));
		}
		...
		{
			pIVR_EnableTurboRecording = tStr.ToBool();
			UE_LOG(LogTemp, Log, TEXT("%s"), *(tStr));
		}
		...
		{
			pIVR_TurboRate = atof(TCHAR_TO_UTF8(*tStr));
			UE_LOG(LogTemp, Log, TEXT("%s"), *(tStr));
		}
		...
		IVR_EnableTurboRecording = pIVR_EnableTurboRecording;
		IVR_TurboRate = pIVR_TurboRate;
	...
	else
	{
		...
		pIVR_Gamma = 1.0f;
		pIVR_EnableTurboRecording = false;
		pIVR_TurboRate = 3.0f;
	...
	float                     pIVR_BaseLookUpRate)
{
	...
	SConfigFile.SetString(SectionName, TEXT("Width"), *FString::FromInt(pIVR_Width));
	SConfigFile.SetString(SectionName, TEXT("Height"), *FString::FromInt(pIVR_Height));
	SConfigFile.SetString(SectionName, TEXT("Gamma"), *FString::SanitizeFloat(pIVR_Gamma));
	SConfigFile.SetString(SectionName, TEXT("EnableTurboRecording"), pIVR_EnableTurboRecording ? TEXT("true") : TEXT("false"));
	SConfigFile.SetString(SectionName, TEXT("TurboRate"), *FString::SanitizeFloat(pIVR_TurboRate));
...
void UIVRCameraManEditorSubsystem::StabilizeVideos()
{
	...
	FString FramesFilePath = FPaths::ConvertRelativePathToFull(FPaths::ProjectDir() / "Plugins" / "IVRCameraMan" / "Recordings");
